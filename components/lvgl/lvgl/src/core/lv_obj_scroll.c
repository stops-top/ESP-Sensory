...
#else
#define SCROLL_ANIM_TIME_MIN    2    /*ms*/
#define SCROLL_ANIM_TIME_MAX    15    /*ms*/
#endif
...
void lv_obj_scroll_by(lv_obj_t * obj, lv_coord_t x, lv_coord_t y, lv_anim_enable_t anim_en)
{
    ...
    if(anim_en == LV_ANIM_ON) {
        ...
        if(x) {
            ...
            uint32_t t = lv_anim_speed_to_time((lv_disp_get_hor_res(d) * 2) >> 1, 0, x);
            ...
            //LV_LOG_USER("scroll_x, start:%d,end:(%d) + (%d) = %d, duration:%d ms", -sx, -sx, x, -sx +x, t);
